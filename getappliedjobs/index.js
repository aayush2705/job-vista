...
// // Helper: Convert blob stream to string
...
// // Helper: Get currently logged-in user's email
...
//             context.res = {
//                 status: 200,
//                 body: [] // Return empty if user has not applied to any job yet
//             };
//             return;
//         }
...
//         const applications = JSON.parse(downloaded);
...
//         };
//     } catch (err) {
//         context.log("Error fetching user's applications:", err.message);
//         context.res = {
//             status: 500,
//             body: "Failed to retrieve applications."
//         };
//     }
// };
const { BlobServiceClient } = require("@azure/storage-blob");
...
module.exports = async function (context, req) {
    try {
        ...
        const downloadBlockBlobResponse = await blockBlobClient.download();
        const downloaded = await streamToString(downloadBlockBlobResponse.readableStreamBody);
        const rawApplications = JSON.parse(downloaded);

        // Exclude 'companyName' from each record
        const applications = rawApplications.map(app => {
            const { companyName, ...rest } = app;
            return rest;
        });
